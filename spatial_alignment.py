...
def spatial_alignment(sensor, object_id):
    ...
    :param sensor: (class) sensor class that contains a list of objects(class),
                   H_sensor_veh transformation matrix
    ...
    :return: x: ((n,) array) state vector in the vehicle coordinate frame
             P: ((nxn) array) covariance matrix of the state x in the 
             vehicle coordinate frame
    """
    obj = sensor.objects[object_id]
    x = np.dot(sensor.H_sensor_veh, np.concatenate((obj.x, [1])))[:-1]
    P = np.dot(np.dot(sensor.H_sensor_veh[:-1, :-1], obj.P), 
                      sensor.H_sensor_veh[:-1, :-1].T)
    return x, P
...
if debug:
    class object:  # dummy object class
        def __init__(self):
            # np.random.random((8,))  # random state vector
            self.x = np.array((2,0,0,0,0,0,0.,0.))
    ...
    class sens:  # dummy example of a sensor class
        def __init__(self):
            ...
            # position of the sensor wrt car center.
            self.delta = np.array((1., np.sqrt(3)))  
            rot_psi = np.array([[np.cos(self.psi), -np.sin(self.psi)], 
                                [np.sin(self.psi), np.cos(self.psi)]])
            H_sensor_veh = np.zeros((9, 9))
            # fill the diagonal matricies:
            for i in range(3): H_sensor_veh[i*2:(i+1)*2, i*2:(i+1)*2] = rot_psi  
    ...
    object_1_state_tranformed, object_1_cov_transformed = spatial_alignment(
                                                            dummy_sensor, 
                                                            object_id)
